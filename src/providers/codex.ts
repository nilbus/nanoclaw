...
registerProviderContainerConfig('codex', (ctx) => {
  ...
  const saddleTokenPath = path.join(process.cwd(), '.github-saddle-view-build.token');
  if (fs.existsSync(saddleTokenPath)) {
    const token = fs.readFileSync(saddleTokenPath, 'utf-8').trim();
    if (token) {
      env.SADDLE_VIEW_BUILD_GITHUB_TOKEN = token;
